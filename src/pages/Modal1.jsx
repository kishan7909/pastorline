...
const Modal1 = ({ children, title = "Modal1", ...props }) => {
  ...
  useEffect(() => {
    setContactsList([]);
    dispatch(GetContacts(companyId, 1, "", debounceSearch));
    setPage(1);
  }, [debounceSearch]);
  ...
  useEffect(() => {
    setContactsList([]);
    dispatch(GetContacts(companyId, page, "", search));
  }, []);

  const handleSubmit = (e) => {
    e.preventDefault();
    setContactsList([]);
    dispatch(GetContacts(companyId, 1, "", search));
    setPage(1);
  };
  ...
  const onScroll = (e) => {
    ...
    if (scrollTop + clientHeight === scrollHeight) {
      dispatch(GetContacts(companyId, page + 1, "", search));
      setPage(page + 1);
    }
  };
